...
install_varnish() {
	...
	cd /etc/apache2
	sed -i -e 's/80$/8008/g' ports.conf
	sed -i -e 's/80>/8008>/g' sites-available/*
	cmd apachectl configtest

	# configuring apache2 for SSL
	# load all the necessary modules
	a2enmod ssl proxy proxy_balancer proxy_http
	local X=sites-available/default-ssl.conf T="
		########################################################################
	...
	sed -i -e 's/"8080"/"8008"/g' /etc/varnish/default.vcl
